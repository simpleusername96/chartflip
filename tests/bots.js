 ...
 * a smart player boosts below `slow` speed (climbs, crawls, after a bad landing), and also spends
 * down a nearly full gauge so coins are not wasted. `sigma` blurs the speed read per decision;
 * `always` holds whenever there is cash.
 */
function booster({ slow = 2600, sigma = 0, always = false } = {}, seed = 1) {
  ...
  return (run, memory) => {
    ...
    // Boost drives the wheels: only worth holding on the ground.
    return run.grounded && Math.hypot(run.vx, run.vy) < slow + memory.error;
  };
}
 ...
 */
function play(course, policy, { rules, limitSeconds = 240, boost = boostPolicies.smart } = {}) {
